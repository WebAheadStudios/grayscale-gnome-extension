  ...
  try {
    ...
    if (windowManager === 'x11') {
      ...
      try {
        ...
        warn('Failed to restart GNOME Shell automatically. Press Alt+F2, type "r", then Enter to restart manually.');
      }
    } else {
...
function installExtension() {
  ...
  if (isSymlink) {
    // Development installation - create symlink
    ...
    // Production installation - copy files
    log('Copying extension files...');
    try {
      cpSync(DIST_DIR, INSTALL_PATH, { recursive: true });
...
async function main() {
  try {
    ...
    // Auto-enable if it's not enabled
    const status = getExtensionStatus();
    if (status !== 'enabled') {
      enableExtension();
    } else {
