...
export class PerformanceMonitor extends GObject.Object {
    ...
    private _nextMeasurementId: number = 1;
    private _nextTimerId: number = 1;
    private _nextAlertId: number = 1;
    ...
    removeThreshold(metric: string, component?: string, operation?: string): boolean {
        const key = this._getThresholdKey({ metric, component, operation } as PerformanceThreshold);
    ...
    getStatistics(
        ...
        for (const [key, measurements] of this._measurements.entries()) {
            ...
            if (filteredMeasurements.length === 0) continue;
    ...
    generateReport(timeRange?: { start: number; end: number }): PerformanceReport {
        ...
        const avgCpu =
            resourceData.length > 0
                ? resourceData.reduce((sum, r) => sum + r.cpu.percentage, 0) / resourceData.length
                : 0;
        const avgMemory =
            resourceData.length > 0
    ...
    private _initialize(): void {
        ...
        if (this._config.enableResourceMonitoring) {
            this._resourceMonitorTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 10000, () => {
    ...
    private _checkThresholds(measurement: PerformanceMeasurement): void {
        ...
        for (const threshold of this._thresholds.values()) {
            ...
            if (threshold.metric !== measurement.name) continue;
            if (threshold.component && threshold.component !== measurement.component) continue;
            if (threshold.operation && threshold.operation !== measurement.operation) continue;
    ...
    private _generateRecommendations(
        ...
        if (resourceData.length > 0) {
            const avgMemoryUsage =
                resourceData.reduce((sum, r) => sum + r.memory.percentage, 0) / resourceData.length;
            const avgCpuUsage =
                resourceData.reduce((sum, r) => sum + r.cpu.percentage, 0) / resourceData.length;
