...
export const Logger = GObject.registerClass(
    ...
    class Logger extends GObject.Object {
        ...
        debug(message: string, data?: any, component = 'system'): void {
            this.log(LogLevel.Debug, message, data, LogCategory.System, component);
        }

        info(message: string, data?: any, component = 'system'): void {
            this.log(LogLevel.Info, message, data, LogCategory.System, component);
        }

        warn(message: string, data?: any, component = 'system'): void {
            this.log(LogLevel.Warn, message, data, LogCategory.System, component);
        }

        error(message: string, data?: any, error?: Error, component = 'system'): void {
        ...
        critical(message: string, data?: any, error?: Error, component = 'system'): void {
            const entryData = { ...data };
            if (error) {
                entryData.error = {
                    name: error.name,
        ...
        performance(operation: string, duration: number, data?: any, component = 'system'): void {
            const perfData = {
                ...data,
                operation,
                duration,
        ...
        enableFileLogging(filename?: string): void {
            try {
                ...
                const file = Gio.File.new_for_path(
                    GLib.build_filenamev([GLib.get_tmp_dir(), logFile])
                );
        ...
        getStatistics(): LogStatistics {
            this._statistics.bufferUtilization =
                (this._buffer.size / this._config.bufferSize) * 100;
            return { ...this._statistics };
        }

        getRecentEntries(count = 50, level?: LogLevel): LogEntry[] {
        ...
        searchLogs(
            ...
            return this._buffer.entries.filter(entry => {
                ...
                const matchesComponent =
                    !options.component || entry.component === options.component;
