...
export class UIController extends GObject.Object implements IUIController, ExtensionComponent {
    ...
    async requestMonitorToggle(monitorIndex: number, source: string = 'api'): Promise<boolean> {
        try {
            ...
            const newState = await this._stateManager.setMonitorState(monitorIndex, !currentState, {
                source,
            });
    ...
    private _connectStateSignals(): void {
        ...
        this._stateManager.connect('settings-changed', (manager: any, key: string, variant: any) =>
            this._handleSettingChange(key, variant)
        );
    }

    private _handleStateChange(globalState: boolean, previousState: boolean, options: any): void {
    ...
    private _showNotification(
        ...
        try {
            ...
            const notification = new (Main.messageTray as any).Notification(source, title, message);
