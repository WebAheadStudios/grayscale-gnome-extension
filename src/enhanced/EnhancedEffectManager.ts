...
export const EnhancedEffectManager = GObject.registerClass(
    { GTypeName: 'GrayscaleEnhancedEffectManager' },
    class EnhancedEffectManager
        extends (BaseComponent as any)
        implements IEffectManager, EnhancedExtensionComponent
    {
        // Inherited from GObject.Object (via BaseComponent) - declared for TypeScript
        declare connect: (signal: string, callback: (...args: any[]) => any) => number;
        declare disconnect: (id: number) => void;
        declare destroy: () => void;
        // Inherited lifecycle methods from BaseComponent
        declare enable: () => void;
        ...
        private _effectPool: EffectPool;
        ...
        private async _applyGlobalEffectInternal(
            ...
            for (let i = 0; i < monitors.length; i += batchSize) {
                ...
                for (const monitor of batch) {
                    results.push(
                        this._applyMonitorEffectWithResult(monitor.index, enabled, options)
                    );
                }
        ...
        private async _applyEffectToMonitor(
            ...
            const effect = this._effectPool.acquire() as Clutter.DesaturateEffect;
        ...
        private _isActorOnMonitor(actor: any, monitorIndex: number): boolean {
            ...
            const [_width, _height] = actor.get_transformed_size();
            const monitor = Main.layoutManager.monitors[monitorIndex];
        ...
        async updateEffect(monitorIndex: number, config: Partial<EffectConfig>): Promise<void> {
            ...
            if (this._activeEffects.has(monitorIndex)) {
                await this.applyMonitorEffect(monitorIndex, true, {
                    animated:
                        config.animationDuration !== undefined && config.animationDuration > 0,
                    duration: config.animationDuration,
                });
            }
        }
        ...
        async toggleMonitorEffect(
            ...
            const currentState =
                this._stateManager?.getMonitorState(monitorIndex)?.enabled || false;
            const targetEnabled = enabled !== undefined ? enabled : !currentState;
...
export type EnhancedEffectManager = InstanceType<typeof EnhancedEffectManager>;
